...
- [x] **(AC: 4)** Create `internal/tasks/task.go` with a simple `Task` struct: `type Task struct { Text string }`.
- [x] **(AC: 2, 3)** Create `internal/tasks/file_manager.go`.
...
- [x] **(AC: 3)** Create `internal/tasks/file_manager_test.go` to test the `LoadTasks` function, including the file creation case.
- [x] **(AC: 5)** Modify `cmd/threedoors/main.go`:
...
- [x] **(AC: 6)** Update the `View()` method in `main.go` to render the three selected tasks, each prefixed with "Door 1:", "Door 2:", and "Door 3:". Use `lipgloss` for basic styling if desired.
- [x] **(AC: 7)** Ensure the 'q' and 'ctrl+c' quit functionality remains intact.
- [x] **(AC: 1)** Manually run the application to verify that it correctly reads from `~/.threedoors/tasks.txt` on subsequent runs.
